...
$('#delperdnsinfo').live('click', function() {
	_this = $(this).parent().parent();
	var nowhr = _this.index(); //获取当前行数
	var toptrnum = _this.attr('datatoptrnum');
	_this.remove();
	var action = "delperdnsinfo";
	var url = "public.html";
	$.ajax({
		type: "POST",
		url: url,
		...
		success: function(data) {
			...
			if (num1 == 0) {
				...
				$('.tablebox table tr:nth-child(' + toptrnum + ') td:nth-child(5) #lproductsbuynum').html(num1);
				$('.fudong li em').html(num2);

			}
			//$('.dnsyouhui').hide();
			var action1 = "vgetalldnsproprice";
			$.ajax({
				type: "POST",
				url: url,
				data: {
				...
				success: function(data) {
					data=data.replace(/\s/g,'');
					var newprice = data.split('*')[0];
					var youhuiprice = data.split('*')[1];
					if (num1 > 0) {
						$('.feiydxs span em').html(newprice);
						$('.tablebox table tr:nth-child(' + toptrnum +
							') td:nth-child(6) #lproductsprice').html(newprice);
					}
					$('.dnsyouhui').html(youhuiprice);
					var isadditionalpro = $('#checkaddpro').is(':checked');
					var azservicenum = $('#azservicenum .shang input').val();
					var checknum = "";
					$('#shopcart tr').each(function() {
...
$('.dnslist td .deldnscart').live('click', function() {
	var _this = $(this).parent().parent();
	var notrnum = _this.index() - 1;
...
$('#allcheck').click(function() {
	...
	if (checkstate) {
		...
		$('.dnslist .checks').attr("checked", "true");
	} else {
		$('.cartlist .checks').removeAttr("checked");
		$('.dnslist .checks').removeAttr("checked");
	}
	var checknum = "";
	$('#shopcart tr').each(function() {
		if ($(this).find('.checks').is(':checked')) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data == 1) {
					...
					tips = "DNS云解析优惠劵存在多次使用的情况，请检查";
					$('#alert1 .sy-content').html(tips);
					syalert.syopen('alert1');
					return false;
				} else if (data == "d7") {
					tips = "DNS云解析优惠码存在多次使用的情况，请检查";
					$('#alert1 .sy-content').html(tips);
					syalert.syopen('alert1');
					return false;
				} else {
...
$('.editdnspaybtn').live('click', function() {
	var _this = $(this).parent().parent();
	var notrnum = _this.index() - 1;
	var url = "public.html";
	var action = "viewdnscartinfo";
	$.ajax({
		type: "POST",
		url: url,
		data: {
...
$('.paybtn').live('click',function(){
	var certid = $(this).attr('dataid');
	var url = "public.html";
	var action = "buynewcertificatecart";    //0922
	$.ajax({
			type: "POST",
			url: url,
			data: {
			...
			success: function(data) {
				...
				// 		theme: "light-thin"
				// 	});
				// 	if($('#bu_buynum .bu_peryear1').length > 0){
				// 		$('#bu_buynum .butips em').show();
				// 		$('#bu_buynum .butips em').removeClass('active');     //0918修改
				// 		$('#bu_buynum .butips em').html('展开');
				// 	}else{
				// 		$('#bu_buynum .butips em').hide();
				// 	}
...
$('#bu_edition a').live('click', function() {
	...
	var certtitle=$('.buycerttitle span').html();
	var regex = /(代码签名|邮件签名|文档签名)/;
	var editiontips;
	if (edition == "体验版") {
	...
	if(regex.test(certtitle)) {
		$('#bu_edition .butips').html('');
	}else{
	...
	var checkyears=$('#bu_buynum .bu_buynumlist a').index($('#bu_buynum .bu_buynumlist a.cur'))+1;
	var isopenbuyyear=$('#bu_buynum .butips em').hasClass('active');
	var signtype = $('#bu_signtype p .cur').attr('datavalue');
	...
	var url = "public.html";
	var action = "changecertcart";  //0922
	$.ajax({
		type: "POST",
		url: url,
		data: {
...
$('#bu_signtype a').live('click', function() {
	$('#bu_signtype a').removeClass('cur');
	$(this).addClass('cur');
	var singledomainnum = parseFloat($('#singledomainnum .shang input').val());
	var multidomainnum = parseFloat($('#multidomainnum .shang input').val());
	var algorithm = $('#bu_algorithm p .cur').attr('datavalue');
...
$('.renewbtn').live('click', function() {
	...
	var activeid = $(this).attr('dataactiveid');
	var url = "public.html";
	var action = "renewcertificatecart";     //0922
	$.ajax({
		type: "POST",
		url: url,
		data: {
		...
		success: function(data) {
			if(data==1){
				...
				$('.buycertlayer').addClass('show');
				if($('#renew_buynum .bu_peryear1').length > 0){
					$('#renew_buynum .butips em').show();
					$('#renew_buynum .butips em').removeClass('active');     //0918修改
					$('#renew_buynum .butips em').html('点击展开');
				}else{
					$('#renew_buynum .butips em').hide();
				}
			}
		}
	})
...
$('#renew_buynum .butips em').live('click',function(){        //0918修改
	if($(this).hasClass('active')){
		$(this).removeClass('active');
		$(this).html('点击展开');
		$('#renew_buynum .bu_peryear1').slideUp();
	}else{
		$(this).addClass('active');
		$(this).html('点击收起');
		$('#renew_buynum .bu_peryear1').slideDown();
	}
})
...
$('#renew_buynum a').live('click', function() {
	...
	var checkyears = $('#renew_buynum .bu_buynumlist a').index($('#renew_buynum .bu_buynumlist a.cur'))+1;
	
	var isopenbuyyear=$('#renew_buynum .butips em').hasClass('active');
	
	var activeid = $('#renewpaybuynow').attr('dataactiveid');
	var url = "public.html";
	var action = "renewprice";     //0922
	$.ajax({
		type: "POST",
		url: url,
		data: {
		...
		success: function(data) {
			...
			if (data == 1) {
				...
				if($('#renew_buynum .bu_peryear1').length > 0){
					$('#renew_buynum .butips em').show();
					if(isopenbuyyear){
						$('#renew_buynum .bu_peryear1').show();
						$('#renew_buynum .butips em').addClass('active');     //0918修改
						$('#renew_buynum .butips em').html('点击收起');
					}else{
						$('#renew_buynum .butips em').removeClass('active');     //0918修改
						$('#renew_buynum .butips em').html('点击展开');
					}
...
$('#renew_certcoupontypecheck a').live('click', function() {
	...
	var procode = $('#renew_buynum p .cur').attr('datavalue');
	var maxyears = $('#renew_buynum p a').length;
	var checkyears = $('#renew_buynum p a').index($('#renew_buynum p a.cur'))+1;
	var isopenbuyyear=$('#renew_buynum .butips em').hasClass('active');
	var activeid = $('#renewpaybuynow').attr('dataactiveid');
	var coupontype = $('#renew_certcoupontypecheck p .cur').attr('datavalue');
	var url = "public.html";
	var action = "newrenewcouponlist";        //0922
	$.ajax({ 
		type: "POST",
		url: url,
		data: {
...
$('#renewpaybuynow').live('click', function() {
	...
	var url = "public.html";
	var action = "renewpayordernow";   //0922
	$.ajax({
		type: "POST",
		url: url,
		data: {
