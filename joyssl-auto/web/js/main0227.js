...
function modifypass(){
	mobile=$('#nmobile').val();
	vercode=mobile+$("#passvercode").val();
...
function checkdomain(){
	...
	$(this).val(domain);
	var checktype=$('#checktype').attr('data-value');
	if(checktype==0){
		tips="请选择验证方式！";
		$('#alert1 .sy-content').html(tips);
		syalert.syopen('alert1');
...
function checkmultidomain(){
	...
	$('#singledomainlist .martb').each(function(){
		var singledomain=$(this).find('input').val();
	...
	$('#wildcarddomainlist .martb').each(function(){
		var multidomain=$(this).find('input').val();
