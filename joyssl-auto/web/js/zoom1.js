...
zoom.prototype = {
    ...
    smallimgClick: function() {
		...
		if($("." + that.smallimg).find('img').width()<300){
			$("." + that.smallimg).click(function() {
				$("." + that.bigimg).css({
					width: smallimgw=$("." + that.smallimg).find('img').width()*1.5,
					height: smallimgh=$("." + that.smallimg).find('img').height()*1.5
				});
				$("." + that.mask).fadeIn();
    ...
    mouseWheel: function() {
        ...
        mousewheel($("." + that.bigimg)[0], function() {
            ...
            if ($("." + that.bigimg).innerWidth() > $(".boxright").width()) {
				tips="不能再放大了哦！";
				$('#alert1 .sy-content').html(tips);
				syalert.syopen('alert1');
                return false;
            }
            if ($("." + that.bigimg).innerHeight() > $(".boxright").height()) {
                tips="不能再放大了哦！";
                $('#alert1 .sy-content').html(tips);
                syalert.syopen('alert1');
                return false;
            }
            var zoomHeight = $("." + that.bigimg).innerHeight() * 1.02;
            var zoomWidth = $("." + that.bigimg).innerWidth() * 1.02;
            $("." + that.bigimg).css({
